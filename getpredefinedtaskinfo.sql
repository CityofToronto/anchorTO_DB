-- FUNCTION: network.getpredefinedtaskinfo(text)

-- DROP FUNCTION network.getpredefinedtaskinfo(text);

CREATE OR REPLACE FUNCTION network.getpredefinedtaskinfo(in_source_class text)
    RETURNS json
    LANGUAGE 'sql'
...
ALTER FUNCTION network.getpredefinedtaskinfo(text)
    OWNER TO network;
