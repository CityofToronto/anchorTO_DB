-- FUNCTION: network.getuserbyid(integer)

-- DROP FUNCTION network.getuserbyid(integer);

CREATE OR REPLACE FUNCTION network.getuserbyid(
	a integer)
    RETURNS SETOF json 
    LANGUAGE 'sql'
...
ALTER FUNCTION network.getuserbyid(integer)
    OWNER TO network;
