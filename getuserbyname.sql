-- FUNCTION: network.getuserbyname(text)

-- DROP FUNCTION network.getuserbyname(text);

CREATE OR REPLACE FUNCTION network.getuserbyname(
	a text)
    RETURNS SETOF json 
    LANGUAGE 'sql'
...
ALTER FUNCTION network.getuserbyname(text)
    OWNER TO network;
